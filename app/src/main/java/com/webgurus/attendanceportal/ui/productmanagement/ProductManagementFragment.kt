...
class ProductManagementFragment  : BaseFragment() , RecyclerViewRoleClickListeners {
    ...
    private fun hitApitoPostCSV(multipartBody: MultipartBody.Part) {
        ...
        val call =
            service.addProductImport("Bearer " + sharedPreferences!!.getString("Access_Token", ""),
                multipartBody)
        // showProgressbar()
        call.enqueue(object : retrofit2.Callback<ImportCustomer?> {
            override fun onResponse(call: Call<ImportCustomer?>, response: Response<ImportCustomer?>) {
                if (response.body() != null) {
                    rl_rootproduct.snackbar(response.body()!!.message.toString())
                }else{
                    rl_rootproduct.snackbar(response.body()!!.message.toString())
                }
        ...
        call.enqueue(object :retrofit2.Callback<ProductListingPojo>{
            override fun onResponse(call: Call<ProductListingPojo?>,
                ...
                if (response.body() != null) {
                    ...
                    for (i in 0 until mListCatg.size){
                        mVariantList.addAll(mListCatg[i].variants)
                    }
                    rv_productmanagement.layoutManager =
                        LinearLayoutManager(activity, RecyclerView.VERTICAL, false)
                    productManagementAdapter =
                        ProductManagementAdapter(requireActivity(), mVariantList!!,this@ProductManagementFragment)
    ...
    private fun showAlertLogout(postion:Int,iD:Int) {
        ...
        { dialogInterface, which ->
            mListCatg.remove(mListCatg.get(postion))
