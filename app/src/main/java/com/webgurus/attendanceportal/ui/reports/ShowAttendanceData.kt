...
class ShowAttendanceData  : BaseActivity() {
    ...
    private fun listeners() {
        ...
        ll_monthly.setOnClickListener(View.OnClickListener {
         ...
         //   hitApitoGettheAttendance(2)
        })
        ll_anually.setOnClickListener(View.OnClickListener {
            changeStatus(3, ll_daily, ll_monthly, ll_anually)
          //  hitApitoGettheAttendance(1)
        })
    }

    private fun hitApitoGettheAttendance(status: Int) {
        x_axisList.clear()
        y_axisList.clear()
        val service = RetrofitClientInstance.getRetrofitInstance().create(GetDataService::class.java)
        val call = service.filterAttendance(
       ...
       // showLoading(true)
        call.enqueue(object : retrofit2.Callback<AttendanceReportPojo?> {
            override fun onResponse(
                call: Call<AttendanceReportPojo?>,
                response: Response<AttendanceReportPojo?>
            ) {
                if (response.body() != null) {
                    hideProgress()
                    x_axisList.addAll(response.body()!!.data.weekLabel)
                    y_axisList.addAll(response.body()!!.data.weekTotal)
                    setData(x_axisList,y_axisList)
                }

            }

            override fun onFailure(call: Call<AttendanceReportPojo?>, t: Throwable) {
                val dsgfd = ""
            }
    ...
    private fun  setData( mXaxisList: ArrayList<String>,yAxisList : ArrayList<Int>) {
        val barWidth: Float
        barWidth = 0.45f
