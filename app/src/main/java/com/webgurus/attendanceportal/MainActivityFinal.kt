...
class MainActivityFinal : AppCompatActivity(), SharedPreferences.OnSharedPreferenceChangeListener {
...
//                .show()
    ...
    override fun onStop() {
        try {
            ...
            //Toast.makeText(this@MainActivityFinal,""+e.toString(),Toast.LENGTH_SHORT).show()
        }
        ...
        if (!Utils.isConnected(this)) {
            ...
            call.enqueue(object : retrofit2.Callback<NewLocationPojo?> {
                override fun onResponse(
                    ...
                    try {
                        ...
                        //    isFiftyClear=true
                            ...
                            startActivity(Intent(this@MainActivityFinal, LoginActivity::class.java))
                            finishAffinity()
                        } else {
                            Toast.makeText(
                                this@MainActivityFinal,
                ...
                override fun onFailure(call: Call<NewLocationPojo?>, t: Throwable) {
                    Toast.makeText(this@MainActivityFinal, t.toString(), Toast.LENGTH_SHORT).show()
                }
            })
        }
    ...
    private fun showAlertLogout() {
        ...
        { dialogInterface, which ->
            ...
            val editor = preferences.edit()
            var settings = this.getSharedPreferences("AcessToken", MODE_PRIVATE)
            if (!settings.getString("LatLongJson", "").equals("")) {
...
//            if (myTempJson.length>0) {
//           // if (!settings.getString("LatLongJson", "").equals("")) {
//                var mList = ArrayList<LocationCustomArray?>()
//                var mListTemp = ArrayList<LocationCustomArray?>()
//                var locationCustomArray = LocationCustomArray()
//               // if(settings.getString("LatLongJson", "")!!.length>0){
//                if(myTempJson.length>0){
//                //    mList = locationCustomArray.changeStringintoList(settings.getString("LatLongJson", "")!!)
//                    mList.clear()
//                   mList = locationCustomArray.changeStringintoList(myTempJson)
//                    if(mList.size>50){
//                        for (i in 50 until mList.size){
//                            mListTemp.add(mList[i])
//                            if(isFiftyClear&&mListTemp.size==50){
//                                readWebPageBulk(locationCustomArray.changeListToJson(mListTemp))
//                                isFiftyClear=false
//                                mListTemp.clear()
//                            }
//                        }
